...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $comm = preg_replace( "/\r\n/", "<BR>", $_POST['comm'] );
        $sql = "UPDATE tb_customercar SET history = CONCAT(history, '<BR>" . $_POST['date'] . " " . $comm . " " . $_POST['medewrk'] . "') WHERE kenteken = '" . $_POST['key'] . "'";
        $stmt = $pdo->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetchAll();

        $sql = "INSERT INTO tb_appointment (kenteken, datum, time_from, time_to, lokatie, soort, leenauto, medewerker) VALUES ('" . $_POST['key'] . "','" . $_POST['date'] . "','" . $_POST['time_from'] . "','" . $_POST['time_to'] . "','" . $_POST['location'] . "','" . $_POST['afspr'] . "','" . $_POST['lcar'] . "','" . $_POST['medewrk'] . "')";
        $stmt = $pdo->prepare($sql);
        $stmt->execute();
        $result = $stmt->fetchAll();
        ...
        foreach($resultcar as $key => $row) {   
            ...
            foreach($resultcustomer as $key2 => $row2) {   
                ...
                $appcust .= "<div class='opmerkingen'><form method='post' enctype='multipart/form-data' name='afspraak' action='afspraak.inc.php?kenteken=" . $row['kenteken'] . "'><label for='comm'>Opmerkingen: </label><textarea id='comm' name='comm' rows='3' cols='50'></textarea><br /><label for='lcar'>Leen auto: </label><select id='lcar' name='lcar'><option value='nee'>nee</option><option value='ja'>ja</option></select><br /><label for='element'>Soort afspraak: </label><select id='element' name='afspr'><option value='apk'>APK</option><option value='grote beurt'>Grote beurt</option><option value='kleine beurt'>Kleine beurt</option><option value='showroom'>Showroom</option><option value='overig'>Overig</option></select><label for='medewrk'><div class='mw'>Medewerker:</label><select id='medewrk' name='medewrk'><option value='medewerker'>Medewerker</option><option value='Johan Vista'>Johan Vista</option><option value='Rick Vista'>Rick Vista</option><option value='Ben Vista'>Ben Vista</option></select></div><br /><label for='lokatie'><div class='pl'>Lokatie:</label><select id='lokatie' name='location'><option value='Maastricht'>Maastricht</option><option value='Sittard'>Sittard</option><option value='Heerlen'>Heerlen</option></select></div><br /><div class='datum'><label for='dat'>Datum: </label><input type='date' name='date' id='dat' required></div><div class='tijd'>Tijd van: <input type='time' name='time_from' id='tm' required> tot <input type='time' name='time_to' id='tm1' required></div><br><br><input type='submit' name='submit' class='verz' value='Verzenden'><input type='hidden' name='key' value='" . $row['kenteken'] . "'></form></div>";
